...
var gq = {
			...
			{	
				...
				{	
					...
					{						
						...
						{
							text[i]['list'][quranBy] = gq.data.quran[quranBy][i];
	...
	load: function (surah, ayah)
	{
		...
		if (this.search.isActive())
		{
			...
			var requestUrl = this.config.api.url;
		...
		if (!this.search.isActive() && this.config.data.enable)
		{		
			if (this.config.data.by == 'juz')
			{
				this.data.ayahList = Quran.ayah.listFromJuz(this.settings.juz);
				requestUrl = this.config.api.url+'all/juz/'+this.settings.juz;
			}
			else if (this.config.data.by == 'surah')
			{
				this.data.ayahList = Quran.ayah.listFromSurah(this.settings.surah);
				requestUrl = this.config.api.url+'all/surah/'+this.settings.surah;
			}
			else
			{
				this.data.ayahList = Quran.ayah.listFromPage(this.settings.page);
				requestUrl = this.config.api.url+'all/page/'+this.settings.page;
			}
		...
		if (!this.config.data.enable && !firstLoad) // if no data need to be output, then run request only once
	...
	bind: {
		 ...
		 * @param isBefore {bool} true for before method execution and false for after method execution
		 * @param $this {object} (optional) - this is useful for places that can be used for $(this)...
		 */
		load: function (name, isBefore, $this)
		{					
		 ...
		 * @returns {boolean} true for success, false on error
		 */
		add: function(id, name, func, isBefore)
		{
		 ...
		 * @returns {boolean} true for success, false on error
		 */
		remove: function (id, name, isBefore)
		{
			if (!id || !name)
				return false; // missing argument or invalid argument
		...
		removeAll: function (id)
		{
			...
			{
				layout.bind.remove(id, name, true);		// delete before
				layout.bind.remove(id, name);			// delete after
			});
		}
	},
