    ...
    switch (data.effect) {
      case 'hard':
        ...
        else if (point.x<csz)
        {
        	...
        	// [ADDED] dont flip last page
        	var turnData = data.opts.turn.data();
			if(turnData.totalPages == data.opts.next)
				return false;
        }
        else
          return false;
  ...
  _showFoldedPage: function(c, animate) {
    ...
    if (folding) {
      ...
      if (animate) {
        ...
        // [MODIFIED] [ADDED] complete page corner turn effect for mouse over
        if(c.corner=="br"  || c.corner=="bl")
        {		
	        this.animatef({
	          from: [point.x, this.height()],
	          to: [c.x, this.height()],
	          duration: 500,
	          frame: function(v) {
	            c.x = Math.round(v[0]);
    	...
    	// [ADDED] corner sizes
		if(c.corner=="bl" || c.corner=="br")
			c.y = this.height();
		else
			c.y = 0;
		if(c.corner=="bl" || c.corner=="tl")
			c.x +=25;
		if(c.corner=="tr" || c.corner=="br")
			c.x -=25;
