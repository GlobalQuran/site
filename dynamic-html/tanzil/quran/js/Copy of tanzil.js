...
var quranTypes = {
    'simple': 'Simple',
    'simple-modified': 'Simple (Enhanced)',
    'simple-min': 'Simple (Minimal)',
    'simple-clean': 'Simple (Clean)',
    'uthmani': 'Uthmani',
    'uthmani-min': 'Uthmani (Minimal)'
};
var defQuranType = 'simple-modified';
var reciteList = {
    'abdulbasit-mrtl': {
        name: 'AbdulBasit',
        base: 'Abdul_Basit_Murattal_192kbps'
    },
    'abdulbasit-mjwd': {
        name: 'AbdulBasit (Mujawwad)',
        base: 'AbdulSamad_64kbps_QuranExplorer.Com'
    },
    'basfar': {
        name: 'Abdullah Basfar',
        base: 'Abdullah_Basfar_192kbps'
    },
    'basfar2': {
        name: 'Abdullah Basfar II',
        base: 'Abdullah_Basfar_32kbps'
    },
    'ajamy': {
        name: 'Ahmad Al-Ajamy',
        base: 'Ahmed_ibn_Ali_al-Ajamy_64kbps_QuranExplorer.Com'
    },
    'ghamadi': {
        name: 'Al-Ghamadi',
        base: 'Ghamadi_40kbps'
    ...
    'husary-mjwd': {
        ...
        base: 'Husary_128kbps_Mujawwad'
    },
    'minshawi': {
        name: 'Al-Minshawi',
        base: 'Menshawi_16kbps'
    },
    'minshawi-mjwd': {
        name: 'Al-Minshawi (Mujawwad)',
        base: 'Minshawy_Mujawwad_192kbps'
    },
    'shateri': {
        name: 'Ash-Shateri',
        base: 'Abu Bakr Ash-Shaatree_128kbps'
    },
    'shuraim': {
        name: 'Ash-Shuraim',
        base: 'Saood bin Ibraaheem Ash-Shuraym_128kbps'
    },
    'sudais': {
        name: 'As-Sudais',
        base: 'Abdurrahmaan_As-Sudais_192kbps'
    },
    'tablawi': {
        name: 'At-Tablawi',
        base: 'Mohammad_al_Tablaway_128kbps'
    },
    'hani': {
        name: 'Hani Rafai',
        base: 'Hani_Rifai_192kbps'
    },
    'akhdar': {
        name: 'Ibrahim Al-Akhdar',
        base: 'Ibrahim_Akhdar_32kbps'
    },
    'muaiqly': {
        name: 'Maher Al-Muaiqly',
        base: 'muaiqly_128kbps'
    },
    'afasy': {
        name: 'Mishari Al-Afasy',
        base: 'Alafasy_128kbps'
    },
    'ayyub': {
        name: 'Muhammad Ayyub',
        base: 'Muhammad_Ayyoub_128kbps'
    },
    'jibreel': {
        name: 'Muhammad Jibreel',
        base: 'Muhammad_Jibreel_128kbps'
    },
    'parhizgar': {
        name: 'Shahriar Parhizgar',
        base: 'Parhizgar_48kbps'
    },
    'ibrahim': {
        name: 'English: Sahih Intl.',
        base: 'English/Ibrahim_Walk_192kbps_TEST',
        trans: 'en.sahih'
    },
    'fooladvand': {
        name: 'Persian: Fooladvand',
        base: 'Fooladvand_Hedayatfar_48Kbps',
        trans: 'fa.fooladvand'
    }
};
var defRecite = 'afasy';
...
var defTrans = 'en.sahih';
var audioTransList = ['en.sahih', 'fa.fooladvand'];
var langList = {
    sq: 'Albanian',
    ar: 'Arabic',
    az: 'Azerbaijani',
...
var TransText = {
    ...
    get: function (ayaCoord, textType) {
        textType = textType || currTrans;
        var index = Quran.getIndex(ayaCoord);
        var text = this.text[currTrans][index];
        return text;
    },
    set: function (ayaCoord, value, textType) {
        textType = textType || currTrans;
...
(function ($) {
    ...
    $.Radio = {
        val: function (name, val) {
            if (isSet(val)) return $('input[name=' + name + '][value=' + val + ']').attr('checked', 'checked');
            else return $('input[name=' + name + ']:checked').val();
        }
    };
    var imagesCache = [];
...
function getTransDir() {
    var lang = getTransLang(currTrans);
...
function toggleCollapse(obj, show) {
    ...
    if (isIE) target.toggle(vis);
    else target[vis ? 'slideDown' : 'slideUp'](300);
}
...
function loadGoogleAnalytics() {
    ...
    $.getScript(url, function () {
        var pageTracker = _gat._getTracker("UA-1019966-12");
...
var Scroller = {
    ...
    scrollToAya: function () {
        var obj = $('#' + currSura + '-' + currAya);
        if (currTab == 'trans') obj = $('#t-' + currSura + '-' + currAya);
        if (obj.length == 0) return;
        if (currAya == 1) obj = obj.prevUntil('span').last();
        var top = obj.offset().top;
        var bottom = top + obj.height();
        if (currTab == 'quran' && showTrans == 'fixed') bottom = Math.max(bottom, $('.tbox').offset().top + $('.tbox').height());
        var wintop = $(document).scrollTop();
        var winh = $(window).height();
        if (top - wintop < winh * this.marginTop || bottom > wintop + winh * (1 - this.marginBottom)) this[isMSafari ? 'goTo' : 'scrollTo'](obj);
    },
    method: 'quadratic',
    startTime: 0,
    target: 0,
...
var fontList = {
    ...
    scheherazade: {
        ...
        file: 'Scheherazade',
        scale: 1.22
    },// http://tanzil.info/res/font/eot/
    //	http://tanzil.info/res/font/org/
    saleem: {
        name: 'PDMS Saleem',
...
var searchPageDirty = false;
var cookieData = {
    currVersion: 1.2,
    currSura: 19,
    currAya: 1,
    currQuran: defQuranType,
    showSigns: true,
    showSmallAlef: true,
    fontID: 'def',
    fontSize: 16,
...
var jsonCookies = ['reciters'];
var cookieExpire = 120;
...
function initPage(start) {
    ...
    readCookies();
    if (start.trans) currTrans = start.trans;
    adjustVariables();
    checkInstalledFonts();
    ...
    setTransFontDefs();
    initUI();
    if (start.page) showPage(start.page);
    if (start.juz) showJuz(start.juz);
    currSura = start.sura || currSura * 1;
    currAya = start.aya || start.sura > 0 || currAya * 1;
    updateFontSettings();
    ...
    $(window).unload(unload).resize(adjustPageHeight);
    initPlayer();
    loadGoogleAnalytics();
}
...
function initUI() {
    ...
    $('#customFont').val(customFont);
    $('#fontSize').val(fontSize);
    $('#textJustify').toggleCheck(textJustify);
    $('#showSigns').toggleCheck(showSigns);
    $('#showSmallAlef').toggleCheck(showSmallAlef);
    $.Radio.val('showTrans', showTrans);
    initAudioUI();
    setAudioTrans();
    initTooltips();
...
function readCookies() {
    for (var i in cookieData) {
        var w = Cookies.get(i);
        if ($.isInArray(i, jsonCookies)) try {
            w = JSON.parse(w);
        } catch (e) {};
        if (w == null) w = cookieData[i];
        if (w == 'true' || w == 'false') w = (w == 'true');
        window[i] = w;
    }
    cookiesReady = true;
}
...
function saveCookies() {
    ...
    for (var i in cookieData) {
        var w = window[i];
        if ($.isInArray(i, jsonCookies)) w = JSON.stringify(w);
        Cookies.set(i, w, cookieExpire);
    }
}

function update(name, val) {
    window[name] = val;
    Cookies.set(name, val, cookieExpire);
}

function adjustDefaults(start) {
    if (start.defTrans) cookieData['currTrans'] = start.defTrans;
    cookieData['showTrans'] = ($(window).width() > 1200) ? 'fixed' : 'hover';
    if (isMSafari) cookieData['showTrans'] = 'none';
}

function adjustVariables() {
    fontSize = fontSize || 16;
    fontID = fontList[fontID] ? fontID : 'def';
    currQuran = quranTypes[currQuran] ? currQuran : defQuranType;
    currTrans = transList[currTrans] ? currTrans : defTrans;
    try {
        reciteList[reciters[0].id].name
    } catch (e) {
        reciters = cookieData.reciters;
    }
    upgrade();
}
...
function upgrade() {
    if (currVersion < '1.1') {
        if (currSura == 6 && currAya == 1) resetCookie('currSura');
        if (currTrans == 'en.shakir') resetCookie('currTrans');
    }
    resetCookie('currVersion');
}

function resetCookie(name) {
    update(name, cookieData[name]);
}
...
function initSuraMenu(sura) {
    ...
    items.push({
        text: i + '. ' + Quran.getSuraName(i, nameType),
        value: i
    });
...
function initFontMenu() {
    ...
    for (var i in fontList) {
        font = fontList[i];
        if (font.embed || font.installed || !font.family) items.push({
            text: font.name,
            value: i
        });
    }
...
function initQuranTypesMenu() {
    initMenuFromHash('typeMenu', quranTypes, currQuran);
}
...
function initMenu1(menuID, items, defVal) {
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        if (defVal && defVal == items[i].value) newMenu.selectedIndex = i;
    }
...
function showMainTab() {
    if (currTab == 'trans') $('#quran-tab').show();
    $('#quranText').toggle(currTab == 'quran');
    $('#transText').toggle(currTab != 'quran');
    $('.tbox').toggle(currTab == 'quran' && showTrans == 'fixed' && !isPageEmbedded);
    $('.tip').hide();
}
...
function showMainTab0(tabID) {
    currTab = tabID;
    $('#quranText').toggle(currTab == 'quran');
    $('#transText').toggle(currTab != 'quran');
    if (currTab == 'trans') $('#quran-tab').children().appendTo('#trans-tab');
    if (currTab == 'quran') $('#trans-tab').children().appendTo('#quran-tab');
    refresh();
}
...
function changeAya(offset, cyclic) {
    var next = Quran.addOffset(currSura, currAya, offset, cyclic);
    displayAya(next);
}

function changeSura(offset) {
    offset = getArrowOffset(offset) || offset;
    if (offset < 0 && currAya > 1) offset = 0;
    showSura(currSura + offset);
}

function changePage(offset) {
    offset = getArrowOffset(offset) || offset;
    var page = $('#pageNum').val() * 1 + offset;
    showPage(page);
}
...
function refresh() {
    currPage = '';
    adjustPageHeight();
    if (currTab == 'search') updateSearchPage(!searchPageDirty);
    else display();
}
...
function displayAya(sura, aya, highlight) {
    ...
    update('currSura', 1 * sura);
    update('currAya', 1 * aya);
    highlightAya = highlight;
    var page = $('#pageNum').val() * 1;
    if (page + ':' + currQuran + ':' + currTrans + ':' + currTab == currPage) updatePage(page);
    else retrievePage(page);
    Player.loadAya();
}

function updatePage(page) {
    var id = (currTab == 'trans' ? 't-' : '') + (currSura + '-' + currAya);
    $('.aya').removeClass('selected');
    $('#' + id).addClass('selected');
    if (currTab == 'quran') TransBox.update();
...
function showPageText(page) {
    ...
    for (i = 0; i < list.length; i++) {
        ...
        if (currSura == sura && currAya == aya) {
            if (highlightAya) ayaText = TextTools.highlight(highlightPattern, ayaText);
            className += highlightAya ? ' highlight' : ' selected';
        }
        ayaText = TextTools.fixText(ayaText, args);
        var trans = TransText.fetch(index);
        if (currTab == 'trans') {
            if (currSura == trans.sura && currAya >= trans.aya && currAya <= trans.ayaTo) {
            ...
            thisAya += '<span class="ayaNumber" onclick="displayAya(' + sura + ', ' + aya + ');">';
            thisAya += '(' + (getTransDir() == 'ltr' ? addr : TextTools.arabicNumber(addr)) + ')</span>';
            var style = '';
            ...
            thisAya += '<span class="ayaNumber">';
            thisAya += '﴿' + TextTools.arabicNumber(aya) + '﴾</span>';
    ...
    var currJuz = Quran.getJuz(currSura, currAya);
    $('#suraName').text((getCurrDir() == 'rtl') ? 'سورة ' + Quran.getSuraName(currSura) : Quran.getSuraName(currSura, 'tname'));
    $('#juzName').text((getCurrDir() == 'rtl') ? 'الجزء ' + TextTools.arabicNumberName(currJuz) : 'Juz ' + currJuz);
    $('.pageNumber').text((getCurrDir() == 'rtl') ? TextTools.arabicNumber(page) : page);
    showTab(currTab);
    initPageActions();
    currPage = page + ':' + currQuran + ':' + currTrans + ':' + currTab;
    updatePage(page);
}
...
function updateQuranSettings() {
    update('currQuran', $('#typeMenu').val());
    update('showSigns', $('#showSigns').isChecked());
    update('showSmallAlef', $('#showSmallAlef').isChecked());
    updateFont();
    refresh();
}

function updateTransSettings() {
    update('showTrans', $.Radio.val('showTrans'));
    TransBox.toggle(showTrans == 'fixed');
    refresh();
}

function setTrans() {
    update('currTrans', $('#transMenu').val());
    setAudioTrans();
    searchPageDirty = true;
    setTransFontDefs();
    refresh();
}

function setAudioTrans() {
    var hasAudio = $.isInArray(currTrans, audioTransList);
    var active = false;
    for (var i in reciters)
    if (reciteList[reciters[i].id].trans == currTrans) active = true;
...
function updateAudioTransSettings() {
    ...
    if (active) {
        for (var i in reciters)
        if (reciteList[reciters[i].id].trans == currTrans) removeReciter(i);
    } else {
        for (var i in reciteList)
        if (reciteList[i].trans == currTrans) addReciter(i);
    }
    setAudioTrans();
}
...
function pageInCache(startAya, endAya) {
    ...
    if (QuranText.get(i) == null) return false;
    return true;
}

function transInCache(startAya, endAya) {
    for (var i = startAya; i < endAya; i++)
    if (TransText.get(i) == null) return false;
    return true;
}
...
function updateSearchPage(notDirty) {
    if (notDirty) {
        currTab = 'search';
        showTab('search');
        initSearchPageActions();
    } else if (searchPattern) retrieveSearchPage();
}

function retrieveSearchPage() {
    var pageOffset = 1 * $('#pageOffset').val();
    var args = {
        type: searchTextType,
        transType: currTrans,
...
function displaySearchPage(results, numAyaMatch, totalMatch) {
    if (results.length == 0) $('#pageOffset').val(0);
    var div = composeSearchPage(results, $('#pageOffset').val());
    $('#searchResults').empty().append(div);
    $('#searchFrame').scrollTop(0);
    initSearchPageActions();
    $('#totalPages').text(Math.ceil(numAyaMatch / resultsPerPage));
    $('#searchStat').text(totalMatch + ' results in ' + numAyaMatch + ' ayas');
    setResultsFont();
...
function composeSearchPage(results, pageOffset) {
    ...
    for (var i = 0; i < results.length; i++) {
        ...
        var div = $('<div />').attr('id', id).addClass('result').html($('<div />').addClass(rowClass).html('<span class="number">' + TextTools.arabicNumber((pageOffset - 1) * resultsPerPage + i + 1) + '. </span>' + line + ' <span class="spec">﴿' + spec + '﴾</span>'));
        mainDiv.append(div);
    }
    return mainDiv;
}
...
function goToAya(sura, aya) {
    currSura = sura, currAya = aya, selectTab('quran');
}

function showSearchPage(offset) {
    var newOffset = 1 * $('#pageOffset').val() + offset;
    newOffset = Math.max(newOffset, 1);
    newOffset = Math.min(newOffset, $('#totalPages').text());
    $('#pageOffset').val(newOffset);
    updateSearchPage();
}
...
function initSearchPageActions() {
...
function initRootMenu(chr) {
    ...
    if (reg.test(Roots[i])) items.push({
        value: Roots[i],
        text: Roots[i]
    });
...
function adjustPageHeight() {
    ...
    var leftPane = $('#left-content').position();
    var footerHeight = $('#footer-content').height();
    var height = winDim.y - leftPane.top - (footerHeight) - 22;
    $('#left-content').css({
...
function initPageActions() {
    ...
    if (currTab == 'quran' && showTrans == 'hover') initTransTip($('.aya'));
}
...
function initTransTip(obj) {
    $(obj).each(function () {
        ...
        $(this).tip({
            text: trans.text,
            title: addr + ' <span>' + transList[currTrans] + '</span>',
            cls: 'transTip',
            delayIn: 800,
            delayOut: 50,
            fadeIn: 60,
...
function initTooltips() {
    if (isMSafari) return;
    $.preloadImages(imgRoot + '/tipsy.gif');
...
function arrowTitle(obj) {
    ...
    return (num == 1) ? 'Next Page' : 'Previous Page';
}
...
function initAudioUI() {
    ...
    initReciteMenu(firstRow, reciters[0]);
    initRepeatMenu();
    initDelayMenu();
    for (var i in reciters) {
        var reciter = reciters[i];
        if (i == 0) initReciteRow(firstRow, reciter);
        else createReciterRow(reciter).appendTo('#reciters');
    }
    $('#playScope').val(playScope);
...
function createReciterRow(reciter) {
    var row = $('<div/>').addClass('menu-row reciter').append($('<select/>').addClass('reciteMenu')).append($('<label/>').addClass('icon icon-close').attr('title', 'Remove'));
...
function initDelayMenu() {
    ...
    items.push({
        text: 'Duration of Aya',
        value: 'len'
    });
    initMenu($('#playDelay'), items, playDelay);
}
...
function addReciter(reciterID) {
    if (reciters.length >= 4) return;
    reciters.push({
        id: reciterID ? reciterID : 'ibrahim',
        num: 1
    });
    var reciter = reciters[reciters.length - 1];
...
function setFontSize(num) {
    var size = $('#fontSize').val() * 1 + num;
    $('#fontSize').val(size);
    updateFontSettings();
}

function updateFontSettings() {
    update('fontSize', $('#fontSize').val() * 1);
    update('textJustify', $('#textJustify').isChecked());
    $('#quranText').css({
        'text-align': textJustify ? 'justify' : 'right'
    });
...
function updateFont() {
    update('fontID', $('#fontMenu').val());
    update('customFont', $('#customFont').val());
    $('#customFontRow').toggle(fontID == 'custom');
    fontList['custom'].family = customFont;
    fontList['custom'].scale = fontWidth(baseFont) / fontWidth(customFont);
    setFont();
}
...
function getFontID() {
    ...
    if (id == 'def') {
        ...
        if (isChrome) id = getPrefInstalledFont();
        if (isMac) id = 'scheherazade';
    }
    return id;
}
...
function checkInstalledFonts() {
    for (var i in fontList) {
        var font = fontList[i];
        if (font.family && fontExists(font.family)) font.installed = true;
    }
}
...
function getPrefInstalledFont(type) {
    ...
    for (var i in list) {
        var fontID = list[i];
        if (fontList[fontID].installed) return fontID;
    }
    return 'arial';
}
...
function setFont() {
    ...
    var font = fontList[fontID];
    if (font.embed) applyEmbedFont(fontID);
    else applyFont(fontID);
}

function applyFont(fontID) {
    if (!fontExists(fontFace(fontID))) fontID = getPrefInstalledFont();
    var font = fontList[fontID];
    currFontID = fontID;
    setQuranFont();
    $('#loading-font').hide();
...
function embedFontStyle(fontID) {
    if ($.isInArray(fontID, embeddedFonts)) return;
    embeddedFonts.push(fontID);
    var font = fontList[fontID];
    var style = "font-family: '" + font.embed + "';" + "src: url('http://tanzil.info/res/font/eot/" + font.file + ".eot');" + "src: local('" + font.family + "'), url('http://tanzil.info/res/font/org/" + font.file + ((font.file == 'KFC_naskh') ? ".otf') format('opentype');" : ".ttf') format('truetype');");
...
TextTools.fixText = function (text, args) {
    ...
    } else text = this.pregReplace('[$HIGH_SALA-$RUB_EL_HIZB$SAJDAH]', '', text);
    if (!args.showSmallAlef) text = this.pregReplace('$SUPERSCRIPT_ALEF', '', text);
...
TextTools.enrichPattern = function (pattern, ignoreHaraka) {
    if (ignoreHaraka) pattern = this.pregReplace("$HARAKA", '', pattern);
...
var Dialog = {
    ...
    init: function (modal) {
        ...
        $('.dbox').bgiframe();
        $.preloadImages(imgRoot + '/dbox-bg.png');
    },
    open: function (args) {
    ...
    loadContent: function () {
        ...
        if ($('#' + arg.id).length) this.display(arg);
        else {
    ...
    setButtons: function () {
        ...
        for (var button in buttons) {
            ...
            $('<div/>').addClass('button').attr('id', id).text(button).click(buttons[button]).disableTextSelect().appendTo($('.dbox-footer'));
        }
    },
...
function openDialog(id, title, width, height) {
    if (/\.php/.test(id)) id = root + '/php/content/' + id;
    Dialog.open({
        id: id,
        title: title,
        modal: -1,
...
var Modal = {
    ...
    init: function () {
        this.blanket = $('<div />').attr('id', 'modal-blanket').css({
            ...
            zIndex: this.zIndex
        }).bgiframe().appendTo(document.body).hide();
        $(window).bind('resize', function () {
            Modal.update();
        });
    },
    show: function (duration, opacity) {
        if (isSet(opacity)) this.blanket.css({
            opacity: this.opacity = opacity
        });
        if (!isSet(duration)) duration = this.duration;
        this.update();
        if (this.opacity >= 0) this.blanket.fadeIn(duration);
...
var TransBox = {
    ...
    toggle: function (show) {
        if (isPageEmbedded || currTab != 'quran') return;
        this.box[show ? 'show' : 'hide'](isIE7 ? null : 200);
    },
    position: function (obj) {
        var top = obj.offset().top;
        var left = $('#right-content').offset().left + $('#right-content').width() - 65;
        this.box.css({
            left: left
        }).css({
            top: top
    ...
    update: function (obj) {
        obj = obj || $('#' + currSura + '-' + currAya);
        if (obj.length < 1) return;
        var trans = getObjTrans(obj);
        if (!trans.text) return;
        var addr = (trans.sura + ':' + trans.aya) + (trans.aya != trans.ayaTo ? '&ndash;' + trans.ayaTo : '');
        var title = addr + ' <span>' + transList[currTrans] + '</span>';
...
(function ($) {
    $.fn.tip = function (op1, op2) {
        if (op1 == 'update') return $.Tip.update(this, op2);
        return $(this).each(function () {
            $.Tip.init(this, op1);
        });
    };
    $.Tip = {
        ...
        init: function (obj, options) {
            ...
            $(obj).bind(op.trigger, function (e) {
                ...
                self.position(e);
                self.show();
                if (op.track) $(e.currentTarget).mousemove(function (e) {
                    $.Tip.position(e)
                });
            }).bind(op.triggerOut, function (e) {
                var op = self.options;
                if (!op.sticky) self.hide();
                if (op.track) $(e.currentTarget).unbind('mousemove');
            });
        },
        show: function () {
            var op = this.options;
            this.showTimer = this.run(function () {
                if (isIE) $.Tip.tooltip.show();
                else $.Tip.tooltip.stop().fadeIn(op.fadeIn, function () {
                    $(this).css({
                        opacity: 1
                    })
                });
        ...
        hide: function () {
            ...
            this.hideTimer = this.run(function () {
                if (isIE) $.Tip.tooltip.hide();
                else $.Tip.tooltip.stop().fadeOut(op.fadeOut);
        ...
        position: function (e) {
            ...
            var xx = op.left ? xr : x + op.offsetX;
            if (xx + w > sr) xx = sr - w;
            xx = Math.max(xx, sl);
            this.tooltip.css({
                left: xx + d,
        ...
        buildTip: function (closeButton) {
            ...
            this.tooltip.empty().append($('<div/>').addClass('tip-wrapper').css({
                position: 'relative'
            }).append($('<div/>').addClass('tip-title').append($((closeButton ? '<a class="tip-close" title="Close" href="#">X</a>' : '') + '<span>Title</span>'))).append($('<div/>').addClass('tip-content')));
...
var Player = {
    ...
    currItem: {},
    delayID: null,
    hasBism: ['abdulbasit-mjwd', 'ajamy', 'parhizgar', 'fooladvand'],
    init: function (forceUseFlash) {
        var hasFlash = swfobject.hasFlashPlayerVersion('8');
        useFlash = isSet(forceUseFlash) ? forceUseFlash : hasFlash;
        $.jPlayer.defaults.nativeSupport = !useFlash
        $.jPlayer.defaults.swfPath = root + '/js/jquery/jplayer';
        $('#player-loading').show();
        var self = this;
        $('<div/>').attr('id', 'jplayer').appendTo('#jplayer-box').jPlayer({
            ready: function () {
                self.player = this;
                self.reset();
                this.volume(1 * currVolume);
                $('#player-pad').fadeIn(200);
                $('#player-loading').hide();
                this.onSoundComplete(function () {
                    Player.proceed()
                });
                this.onProgressChange(function (lp) {
                    Player.progress(lp)
                });
            }
        });
        ...
        $('.jp-volume-toggle').click(function () {
            self.prevVolume = self.player.getData('volume');
    ...
    togglePlay: function () {
        if (this.player.getData('diag.isPlaying')) this.player.pause();
        else if (this.isPlaying) changeAya(+1, true);
        else this.play();
    },
    volume: function () {
        return this.player ? this.player.getData('volume') : this.prevVolume;
    },
    reset: function () {
    ...
    loadAya: function () {
        ...
        if (this.loadedAya == currSura + ':' + currAya) return;
        this.loadedAya = currSura + ':' + currAya;
    ...
    load: function () {
        ...
        if (this.buffer && this.preloadEnabled) this.buffer.clearFile();
        var play = this.isPlaying || this.player.getData('diag.isPlaying');
        this.isPlaying = false;
        this.player.setFile(this.getUrl(this.currItem));
        $('.jp-load-bar').width('100%');
        if (play) this.play();
    },
    preload: function (item) {
        if (!this.buffer || !this.preloadEnabled || playScope == 'aya') return;
        var certificate = item.sura + ':' + item.aya + ':' + item.reciter
        if (this.preloadedAya != certificate) this.buffer.setFile(this.getUrl(item));
        this.preloadedAya = certificate;
    },
    progress: function (lp) {
        if (lp == 100) {
            var next = Quran.getNextAya(this.currItem.sura, this.currItem.aya);
            if (next.sura != 115) this.preload($.extend(next, {
    ...
    proceed: function () {
        ...
        if (playScope != 'cont') {
            var next = Quran.addOffset(currSura, currAya, +1, true);
            var stop = playScope == 'aya' || playScope == 'sura' && next.sura != currSura;
            stop = stop || playScope == 'page' && Quran.getPage(next.sura, next.aya) != Quran.getPage(currSura, currAya);
            stop = stop || playScope == 'juz' && Quran.getJuz(next.sura, next.aya) != Quran.getJuz(currSura, currAya);
            if (stop) return;
        }
        this.isPlaying = true;
        var playDuration = this.player.getData('diag.totalTime');
        var delay = (playDelay == 'len') ? playDuration : playDelay * 1000;
        clearTimeout(this.delayID);
        this.delayID = setTimeout('changeAya(+1, true)', delay);
    },
    getUrl: function (item) {
        fileName = $.pad(item.sura, 3) + $.pad(item.aya, 3) + '.mp3';
        var base = 'http://tanzil.net/res/audio/' + item.reciter + '/';
        if (reciteList[item.reciter].server == 'everyayah.com') base = 'http://www.everyayah.com/data/' + reciteList[item.reciter].base + '/';
    ...
    resetPlayList: function () {
        ...
        for (var i in reciters) {
            var reciter = reciters[i];
            if (i == 0 && currAya == 1 && currSura != 1 && currSura != 9 && !this.bismIncluded(reciter.id)) this.playList.push({
                sura: currSura,
                aya: 0,
                reciter: reciter.id
            });
